...
type Optimizer interface {
	...
	//
	// Run(l *Node, size int, grad func(int) float64, add func(int, float64), learningRate float64) error
	Run(*badstudent.Node, int, func(int) float64, func(int, float64), float64) error
	...
	Save(*badstudent.Node, badstudent.Operator, string) error

	// given a path to the directory (and the name of it, ending without '/'),
	// should recreate the Optimizer with the information given
	//
	// the directory will not be created, used, or altered by anything else
	Load(*badstudent.Node, badstudent.Operator, string, []interface{}) error
}
